...
ChatLogic::~ChatLogic()
{
    //task 5 delete chatbot instance
    //delete _chatBot;

    // Task 3: It's a smart pointer now, so we don't have to manually delete
    // for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    // {
    //     delete *it;
    // }

    // task 4: no longer a member variable so kill it
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        {
                            // Task 3: get iterator on incoming and outgoing node via ID search
                            ...
                            // task 3: create new edge
                            std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id);
                            edge->SetChildNode(childNode->get());
                            edge->SetParentNode(parentNode->get());
                            // task 4: no longer a member variable
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        // task 3: search for nodes which have no incoming edges
